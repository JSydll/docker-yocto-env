...
function build_base_image()
{
  ...
  local init_dir="$(realpath --relative-to="${PROJECT_ROOT}" "${DOCKER_DIR}/init.d")"
  ...
  if [[ "$?" == "0" ]]; then echo ">> Successfully build image. You can find it using the command 'docker images'."; else echo ">> Image building failed."; exit 1; fi
